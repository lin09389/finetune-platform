...
export const TrainingChart: React.FC<TrainingChartProps> = ({ taskId, autoConnect = true }) => {
  ...
  const connectWebSocket = useCallback(() => {
    ...
    const wsUrl = `${wsBase}/training/ws/${taskId}`;
    ...
    ws.onmessage = (event: MessageEvent) => {
      try {
        ...
        if (message.type === 'event' && message.event === 'training_completed') {
          setLoading(false);
        }
      } catch (e) {
    ...
    return (
      <ResponsiveContainer width="100%" height={300}>
        <LineChart data={chartData}>
          ...
          <XAxis dataKey="step" stroke="#888" label={{ value: 'Step', position: 'insideBottom', offset: -5 }} />
          <YAxis stroke="#888" label={{ value: yAxisLabel, angle: -90, position: 'insideLeft' }} domain={['auto', 'auto']} />
          <Tooltip
            contentStyle={{ backgroundColor: 'rgba(0, 0, 0, 0.8)', border: '1px solid #333', borderRadius: 4 }}
      ...
      extra={
        currentProgress && (
          <div style={{ fontSize: 12, color: '#888' }}>
            Epoch: {currentProgress.epoch} | Step: {currentProgress.step}/{currentProgress.total_steps} | ETA:{' '}
        ...
        <>
          {currentProgress && (
            <div style={{ marginBottom: 16, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
              <div style={{ fontSize: 14 }}><strong>Loss:</strong> {currentProgress.loss.toFixed(6)}</div>
              <div style={{ fontSize: 14 }}><strong>LR:</strong> {currentProgress.lr.toExponential(2)}</div>
              <div style={{ fontSize: 14 }}><strong>VRAM:</strong> {currentProgress.vram_used.toFixed(2)} GB</div>
              <div style={{ fontSize: 14 }}><strong>Elapsed:</strong> {Math.round(currentProgress.elapsed_time)}s</div>
            </div>
          )}
          ...
          <Tabs
            ...
            items={[
              { key: 'loss', label: 'Loss 曲线', children: renderChart('loss', '#8884d8', 'Loss') },
              { key: 'lr', label: '学习率', children: renderChart('lr', '#82ca9d', 'Learning Rate') },
              { key: 'vram', label: '显存使用', children: renderChart('vram_used', '#ffc658', 'VRAM (GB)') },
            ]}
          />
        </>
      )}
