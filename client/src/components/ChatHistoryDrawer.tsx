import { Drawer, List, Button, Input, Empty, Typography, Tag, Space } from 'antd'
...
const ChatHistoryDrawer: React.FC<ChatHistoryDrawerProps> = ({
  ...
  const [selectedOutcomeIndexBySession, setSelectedOutcomeIndexBySession] = useState<Record<string, number>>({})

  const filteredSessions = sessions.filter((session) =>
    session.title.toLowerCase().includes(searchTerm.toLowerCase())
  )
  const prioritizedSessions = [...filteredSessions].sort((left, right) => {
    const leftOutcomes = Array.isArray(left.metadata?.task_outcomes) ? left.metadata.task_outcomes.length : 0
    const rightOutcomes = Array.isArray(right.metadata?.task_outcomes) ? right.metadata.task_outcomes.length : 0
                  ...
                  <List.Item.Meta
                    ...
                    description={
                      <div style={{ fontSize: 12, color: '#999' }}>
                        ...
                        {session.model_id && <span style={{ marginLeft: 8 }}>· {session.model_id}</span>}
                        {latestOutcome ? (
                          <div style={{ marginTop: 6 }}>
                            ...
                            <div>
                              <Text type="secondary" ellipsis style={{ maxWidth: 220, display: 'inline-block' }}>
                                {String(latestOutcome.title || latestOutcome.summary || 'Latest task outcome')}
                              </Text>
                            </div>
                            <Button
                              type="link"
                            ...
                            >
                              {expandedSessionId === session.id ? 'Hide outcome' : 'Preview outcome'}
                            </Button>
                                    ...
                                    {taskOutcomes.map((outcome, index) => {
                                      ...
                                      return (
                                        <Button
                                          ...
                                          size="small"
                                          type={selectedOutcomeIndex === index ? 'primary' : 'default'}
                                          onClick={(event) => {
                                            event.stopPropagation()
