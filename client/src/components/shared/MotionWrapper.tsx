...
import { useMotion, containerVariants, itemVariants, scaleItem, itemFromLeft } from '../../hooks/useMotion'
...
export function MotionItem({ children, variant = 'up', className, style, onClick }: MotionItemProps) {
  const { skip } = useMotion()
  if (skip) return <div className={className} style={style} onClick={onClick}>{children}</div>
  return (
    <motion.div
      className={className}
      style={style}
...
export function MotionCard({ children, className, style, onClick, lift = false }: MotionCardProps) {
  ...
  return (
    <motion.div
      ...
      onClick={onClick}
      whileHover={lift ? { y: -3, boxShadow: 'var(--shadow-lg)' } : { borderColor: 'var(--border-hover)' }}
      whileTap={onClick ? { scale: 0.99 } : undefined}
...
export function MotionButton({ children, className, style, onClick, disabled }: MotionButtonProps) {
  const { skip } = useMotion()
  if (skip || disabled) return <div className={className} style={style} onClick={!disabled ? onClick : undefined}>{children}</div>
  return (
    <motion.div
      className={className}
      style={style}
