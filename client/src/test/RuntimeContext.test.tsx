...
const RuntimeProbe: React.FC = () => {
  ...
  return (
    <div>
      ...
      <div data-testid="observed-backend-count">{String(runtime.observed.inference.backends.length)}</div>
      ...
      <button onClick={() => runtime.setInferenceSelection({ backend: 'ollama', modelId: 'llama3:8b' })}>
        set-inference
      </button>
      <button onClick={() => runtime.actions.setInferenceSelection({ backend: 'ollama', modelId: 'action-llama' })}>
        action-set-inference
      </button>
      <button onClick={() => runtime.actions.refreshBootstrap()}>
        refresh-bootstrap
      </button>
      <button onClick={() => runtime.syncInferenceSelection({ backend: 'ollama', modelId: 'shared-ollama' })}>
        sync-inference
      </button>
...
const RuntimePropagationHarness: React.FC = () => {
  ...
  return (
    <div>
      ...
      <button onClick={() => runtime.setInferenceSelection({ backend: undefined, modelId: undefined })}>
...
describe('RuntimeContextProvider', () => {
  ...
  it('renders runtime panel using shared summary data', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('知识库嵌入模型尚未就绪，检索和上传后的向量化能力会受限。')).toBeInTheDocument()
  ...
  it('syncs knowledge selection back into chat settings through the shared runtime API', async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateChatSettings).toHaveBeenCalledWith({ knowledgeCollection: 'shared-sync-docs' })
  ...
  it('syncs inference selection back into chat settings through the shared runtime API', async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateChatSettings).toHaveBeenCalledWith({ backend: 'ollama', modelId: 'shared-ollama' })
      expect(screen.getByTestId('runtime-backend')).toHaveTextContent('ollama')
  ...
  it('exposes grouped runtime actions that match legacy selection and sync behavior', async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateChatSettings).toHaveBeenCalledWith({ knowledgeCollection: 'action-sync-docs' })
