  ...
  beforeEach(() => {
    ...
    mockFetch.mockImplementation((url: string) => {
      if (url.includes('/model-center/source')) {
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ current_source: 'modelscope', default_source: 'modelscope' }) })
      }
      if (url.includes('/model-center/suggestions')) {
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ suggestions: [], default_source: 'modelscope' }) })
      }
      ...
      if (url.includes('/context/projects')) {
        return Promise.resolve({ ok: true, json: () => Promise.resolve({ success: true, projects: [] }) })
      }
      return Promise.resolve({ ok: true, json: () => Promise.resolve({}) })
    })
  })
