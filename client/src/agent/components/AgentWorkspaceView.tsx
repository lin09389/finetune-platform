...
import polish from './AgentWorkspaceView.module.css';
...
function CompactEmpty({ description }: { description: string }) {
  return (
    <div className={polish.compactEmpty} data-compact-empty-state="true" aria-label={description}>
...
export default function AgentWorkspaceView({
  ...
  if (!workspace) {
    return (
      <div className={`${styles.panelEmpty} ${polish.emptySurface}`}>
        <CompactEmpty description="暂无工作区数据" />
      </div>
    );
  }
    ...
    return (
      <section className={styles.workspacePanel} aria-label="变更文件">
        <div className={styles.panelHeader}>
          变更文件 <span>{workspace.changed_files.length}</span>
        </div>
        {workspace.changed_files.length === 0 ? (
          <CompactEmpty description="尚未修改文件" />
        ) : (
          <div className={styles.fileWorkspace}>
              ...
              {workspace.changed_files.map((file) => (
                ...
                >
                  ...
                  <span>
                    ...
                    <small>{file.summary || file.status}</small>
                  </span>
                  <Tag>{file.status}</Tag>
                </button>
              ))}
            ...
            <div className={styles.fileEditor}>
              {!selectedFile ? (
                <CompactEmpty description="选择文件以预览和编辑" />
              ) : fileLoading ? (
                <Spin />
              ) : (
                <>
  ...
  if (tab === 'diff') {
    const fileChanges = workspace.artifacts.filter(
      (artifact) => artifact.artifact_type === 'file_change',
    );
    return (
      <section className={styles.workspacePanel} aria-label="文件差异">
        <div className={styles.panelHeader}>
          文件差异 <span>{fileChanges.length}</span>
        </div>
        {fileChanges.length === 0 ? (
          <CompactEmpty description="尚无可显示的文件差异" />
        ) : (
  ...
  if (tab === 'plan') {
    ...
    return (
      <section className={styles.workspacePanel} aria-label="执行计划">
        <div className={styles.panelHeader}>
          执行计划 <span>{workspace.execution_plan?.status || '未规划'}</span>
        </div>
        {nodes.length === 0 ? (
          <CompactEmpty description="暂无执行计划" />
        ) : (
          <>
            <div className={styles.planSummary}>
              ...
              />
              <span>
                {completed} 已完成 · {nodes.length - completed - blocked} 进行中 · {blocked} 阻塞
              </span>
            </div>
                  ...
                  <div className={styles.planNodeBody}>
                    ...
                    <div className={styles.planMeta}>
                      ...
                      {node.recovery_attempts ? (
                        <span>恢复: {node.recovery_attempts} 次</span>
                      ) : null}
                    </div>
  ...
  if (tab === 'subagents') {
    ...
    return (
      <section className={styles.workspacePanel} aria-label="子 Agent">
        <div className={styles.panelHeader}>
          子 Agent <span>{tasks.length}</span>
        </div>
        {tasks.length === 0 ? (
          <CompactEmpty description="暂无子 Agent 任务" />
        ) : (
          <div className={styles.denseList}>
            {tasks.map((task) => (
  ...
  return (
    <section className={styles.workspacePanel} aria-label="Agent 产物">
      <div className={styles.panelHeader}>
        产物与下一步 <span>{workspace.artifacts.length}</span>
      </div>
