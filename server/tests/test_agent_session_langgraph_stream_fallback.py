...
def test_agent_session_langgraph_keeps_streaming_path_on_old_processor(tmp_path: Path, monkeypatch):
    ...
    async def should_not_use_graph():
        raise AssertionError("graph path should stay disabled when streaming is available")
    ...
    monkeypatch.setattr(service, "_get_graph_runner", should_not_use_graph)
