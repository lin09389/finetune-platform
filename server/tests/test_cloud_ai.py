...
测试场景�?1. API Key 存储和管�?2. 云端 AI 流式聊天
3. 前端集成调用
"""
import sys
import os
...
def test_cloud_ai_api_key_management():
    ...
    try:
        ...
        print(f"  �?API Key 已存储，ID: {test_key_id}")
        
        retrieved_key = secure_storage.get_api_key(test_key_id)
        assert retrieved_key == test_key, "存储�?Key 与检索的不匹�?
        print(f"  �?API Key 检索成�?)
        ...
        print(f"  �?元数据正�? provider={test_provider}, group_id={test_group_id}")
        
        keys = secure_storage.list_api_keys()
        assert any(k["id"] == test_key_id for k in keys)
        print(f"  �?Key 列表中包含新 Key")
...
def test_frontend_api_integration():
    ...
    if os.path.exists(client_api_path):
        with open(client_api_path, 'r', encoding='utf-8') as f:
            content = f.read()
        ...
        print(f"  �?前端 API 文件不存�? {client_api_path}")
        return False
...
def test_chat_page_integration():
    ...
    if os.path.exists(chat_page_path):
        ...
        checks = [
            ("useCloudAI", "云端 AI 开关状�?),
            ("cloudAIConfig", "云端 AI 配置"),
            ("sendCloudMessage", "云端消息发送函�?),
        ...
        print(f"  �?Chat 页面不存�? {chat_page_path}")
        return False
...
def main():
    ...
    results.append(("Provider 初始�?, test_provider_initialization()))
    ...
    if passed_count == total_count:
        print("\n🎉 所有测试通过！云�?AI 调用功能已正确集成�?)
        print("\n使用说明:")
        print("  1. �?AI 对话页面点击 '☁️ 云端' 按钮切换到云�?AI")
        print("  2. 首次使用需要配�?API Key（支�?MiniMax、GLM 等）")
        print("  3. 选择模型后即可开始对�?)
