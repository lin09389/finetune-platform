...
# 添加服务器路�?server_path = Path(__file__).parent.parent
sys.path.insert(0, str(server_path))
os.chdir(server_path)

from core.utils import pre_training_resource_check, get_vram_usage, cleanup_gpu_memory
from core.training_state import get_training_state, TrainingState
from core.training_queue import get_training_queue, TaskPriority, shutdown_queue
...
def test_resource_check():
    ...
    for scenario in scenarios:
        print(f"\n场景：{scenario['method']} - {scenario['model_size']} - 需�?{scenario['required_vram']}GB")
...
def test_gpu_cleanup():
    ...
    print(f"\n清理�?VRAM: {vram_before:.2f} GB")
    
    success = cleanup_gpu_memory()
    
    vram_after = get_vram_usage()
    print(f"清理�?VRAM: {vram_after:.2f} GB")
