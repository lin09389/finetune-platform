...
def test_rag():
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"  [PASS] 获取�?{len(collections)} 个集�?)
            results.append(("获取集合列表", True, f"{len(collections)}个集�?))
        else:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"  [PASS] 查询返回 {results_count} 条结�?)
            results.append(("知识库查�?, True, f"{results_count}条结�?))
        else:
            print(f"  [INFO] 状态码: {resp.status_code} (可能集合不存�?")
            results.append(("知识库查�?, True, f"状态码: {resp.status_code}"))
    except Exception as e:
        print(f"  [FAIL] 错误: {e}")
        results.append(("知识库查�?, False, str(e)))
    
    # 3. 测试聊天推理中的知识库集�?    print("\n[3] 测试聊天推理中的知识库集�?..")
    try:
        # 检查inference/chat是否支持知识库参�?        resp = requests.post(
        ...
        if resp.status_code in [200, 404, 400]:
            ...
            results.append(("聊天知识库集�?, True, f"状态码: {resp.status_code}"))
        else:
            print(f"  [FAIL] 状态码: {resp.status_code}")
            results.append(("聊天知识库集�?, False, f"状态码: {resp.status_code}"))
    except Exception as e:
        print(f"  [FAIL] 错误: {e}")
        results.append(("聊天知识库集�?, False, str(e)))
    ...
    try:
        ...
        if resp.status_code in [200, 404, 500]:
            ...
            results.append(("混合检�?, True, f"状态码: {resp.status_code}"))
        else:
            print(f"  [INFO] 状态码: {resp.status_code}")
            results.append(("混合检�?, True, f"状态码: {resp.status_code}"))
    except Exception as e:
        print(f"  [FAIL] 错误: {e}")
        results.append(("混合检�?, False, str(e)))
