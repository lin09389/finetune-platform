...
async def search_documents(request: SearchRequest, collection_id: str = Form(...)):
    ...
    try:
        ...
        # 组装上下�?        context = rag_service.search_with_context(
            collection_name=collection_id,
            query=request.query,
            top_k=request.top_k
        )
...
async def rag_chat(request: RAGChatRequest):
    ...
    try:
        ...
        # 1. 检索相关文�?        results = rag_service.search(
        ...
        # 3. 构建提示�?        system_prompt = request.system_prompt or """你是一个有帮助的助手。请基于以下上下文回答问题�?如果上下文中没有相关信息，请说明你不知道�?""
...
回答�?""
        ...
        # 4. 调用 LLM（使用现有的推理服务�?        # 这里调用 Ollama 或其他后�?        import requests
...
class HybridSearchRequest(BaseModel):
    ...
    vector_weight: Optional[float] = Field(default=0.5, ge=0, le=1, description="向量检索权�?)
    keyword_weight: Optional[float] = Field(default=0.5, ge=0, le=1, description="关键词检索权�?)
...
async def hybrid_search(
    ...
    """
    混合检�?    
    - 结合向量检索和 BM25 关键词检�?    - 支持 RRF 和加权融合两种方�?    - 可调节向�?关键词权�?    """
    try:
        rag_service = get_rag_service()
...
class RerankRequest(BaseModel):
    """重排序请�?""
    query: str = Field(..., description="查询文本")
    results: List[Dict[str, Any]] = Field(..., description="待重排序的检索结�?)
    top_k: Optional[int] = Field(default=None, description="返回数量")
    threshold: Optional[float] = Field(default=None, ge=0, le=1, description="分数阈�?)
...
async def rerank_results(request: RerankRequest):
    ...
    - 使用 Cross-Encoder 模型计算相关�?    - 支持设置分数阈值过滤低质量结果
    """
    try:
...
class SearchAndRerankRequest(BaseModel):
    """检索并重排序请�?""
    query: str = Field(..., description="查询文本")
    top_k: int = Field(default=5, ge=1, le=20, description="最终返回数�?)
    retrieval_top_k: int = Field(default=20, ge=1, le=50, description="初始检索数�?)
    use_hybrid: bool = Field(default=True, description="是否使用混合检�?)
    vector_weight: float = Field(default=0.5, ge=0, le=1, description="向量检索权�?)
    keyword_weight: float = Field(default=0.5, ge=0, le=1, description="关键词检索权�?)
...
async def search_and_rerank(
    ...
    """
    检索并重排序（一体化接口�?    
    - 先进行混合检索或向量检�?    - 再使�?Cross-Encoder 重排�?    - 返回最终的高质量结�?    """
    try:
        rag_service = get_rag_service()
        hybrid_retriever = get_hybrid_retriever(
            vector_store=rag_service.vector_store,
...
class EvaluateRequest(BaseModel):
    ...
    retrieved_ids_list: List[List[str]] = Field(..., description="各查询的检索结�?ID 列表")
    relevant_ids_list: List[List[str]] = Field(..., description="各查询的相关文档 ID 列表")
    k_values: Optional[List[int]] = Field(default=[1, 3, 5, 10], description="评估�?K �?)
...
async def evaluate_retrieval(request: EvaluateRequest):
    """
    评估检索质�?    
    - 计算 MRR、MAP、Precision@K、Recall@K、NDCG@K 等指�?    - 支持批量评估多个查询
    """
    try:
...
class FeedbackRequest(BaseModel):
    """用户反馈请求"""
    query: str = Field(..., description="查询文本")
    retrieved_ids: List[str] = Field(..., description="检索结�?ID 列表")
    clicked_ids: List[str] = Field(..., description="用户点击的文�?ID 列表")
    relevant_ids: Optional[List[str]] = Field(default=None, description="用户标记的相关文�?ID")
...
async def record_feedback(request: FeedbackRequest):
    ...
    try:
        ...
        return {"message": "反馈已记�?, "query": request.query}
