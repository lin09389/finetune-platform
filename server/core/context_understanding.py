...
class PronounResolver:
    ...
    def _resolve_pronoun(
        ...
        if pronoun_type == PronounType.PERSONAL:
            if pronoun in ["我", "i", "me"]:
                return None
            elif pronoun in ["你", "you"]:
                return None
            else:
                for entity in reversed(entities):
...
class OmissionCompleter:
    ...
    def _get_last_user_message(self, history: List[Message]) -> Optional[Message]:
        """获取最后一条用户消息"""
...
_engine_instance: Optional[ContextUnderstandingEngine] = None
