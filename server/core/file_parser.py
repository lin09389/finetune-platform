...
class FileParser:
    ...
    async def parse_file(
        ...
        logger.info(f"开始解析文件: {filename}, 类型: {file_type.value}, 大小: {file_size}")
    ...
    async def _extract_word(self, file_path: str) -> str:
        ...
        try:
            ...
            raise ImportError(
                "Word 文档解析需要安装 python-docx: pip install python-docx"
            )
...
class FileVectorService:
    ...
    async def index_file(self, parsed_file: ParsedFile) -> tuple[str, list[str]]:
        ...
        if not parsed_file.chunks:
            logger.warning(f"文件没有内容块: {parsed_file.file_id}")
            return "", []
        ...
        logger.info(f"文件向量化完成: {parsed_file.filename}, 集合: {collection_name}")
    ...
    async def delete_file_vectors(self, file_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"已删除文件向量集合: {collection_name}")
            return True
        except Exception as e:
