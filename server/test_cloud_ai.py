...
def test_cloud_ai():
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            for p in providers:
                print(f"    - {p['id']}: {p['name']} ({len(p['models'])} 个模�?")
            results.append(("获取服务商列�?, True, f"{len(providers)}个服务商"))
        else:
            print(f"  [FAIL] 状态码: {resp.status_code}")
            results.append(("获取服务商列�?, False, f"状态码: {resp.status_code}"))
    except Exception as e:
        print(f"  [FAIL] 错误: {e}")
        results.append(("获取服务商列�?, False, str(e)))
    ...
    # 3. 测试API Key验证（无Key时的错误处理�?    print("\n[3] 测试API Key验证（无Key时的错误处理�?..")
    try:
        resp = requests.post(
    ...
    for provider in ["minimax", "glm"]:
        try:
            ...
            if resp.status_code == 200:
                ...
                print(f"  [PASS] {provider}: {len(models)} 个模�?)
                results.append((f"获取{provider}模型列表", True, f"{len(models)}个模�?))
            else:
    ...
    print("  �?Minimax Coding - 编程专用，代码生�?优化")
    print("  �?GLM/智谱AI - 中文能力�?)
    print("\n功能特�?")
    print("  �?API Key 加密存储")
    print("  �?非流式聊�?)
