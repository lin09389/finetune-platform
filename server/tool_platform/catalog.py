...
class ToolProjectionConstraints(_CatalogModel):
    ...
    @field_serializer("required_provider_facts", "required_model_facts", "required_platform_facts")
...
class ToolCatalogEntry(_CatalogModel):
    ...
    def serialize_schema(self, value: FrozenJsonObject) -> JsonValue: return _jsonable(value)

class ToolCatalogSnapshot(_CatalogModel):
    schema_version: Literal[1] = 1
    tools: tuple[ToolCatalogEntry, ...] = ()

def catalog_json(catalog: Sequence[ToolCatalogEntry] | ToolCatalogSnapshot) -> str:
    payload: JsonValue = catalog.model_dump(mode="json") if isinstance(catalog, ToolCatalogSnapshot) else [item.model_dump(mode="json") for item in catalog]
...
__all__ = ["ToolCatalogEntry", "ToolCatalogSnapshot", "ToolProjectionConstraints", "catalog_json"]
