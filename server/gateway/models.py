...
class GatewayMessage(BaseModel):
    ...
    correlation_id: Optional[str] = Field(None, description="关联 ID（用于请�?响应匹配�?)


class GatewayResponse(BaseModel):
    """Gateway 响应"""
    id: str = Field(..., description="响应 ID")
    correlation_id: str = Field(..., description="关联的请�?ID")
...
class DeviceInfo(BaseModel):
    ...
    status: DeviceStatus = Field(DeviceStatus.ONLINE, description="设备状�?)
    ip_address: Optional[str] = Field(None, description="IP 地址")
    user_agent: Optional[str] = Field(None, description="用户代理")
    last_seen: datetime = Field(default_factory=datetime.now, description="最后活跃时�?)
...
class AgentInfo(BaseModel):
    ...
    status: str = Field("idle", description="状�?)
    created_at: datetime = Field(default_factory=datetime.now, description="创建时间")
    last_active: datetime = Field(default_factory=datetime.now, description="最后活跃时�?)
...
class HeartbeatConfig(BaseModel):
    ...
    max_retries: int = Field(3, description="最大重试次�?)
