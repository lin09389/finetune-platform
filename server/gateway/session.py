...
class GatewaySessionManager:
    ...
    def create_session(self, device_id: str, metadata: Optional[Dict[str, Any]] = None) -> DeviceSession:
        """创建设备会话"""
    ...
    def export_session_data(self, device_id: str) -> Optional[Dict[str, Any]]:
        """导出会话数据"""
        session = self._sessions.get(device_id)
        if not session:
            return None
...
_session_manager: Optional[GatewaySessionManager] = None
