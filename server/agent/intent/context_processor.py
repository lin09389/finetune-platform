...
class DialogueMemory:
    ...
    def get_recent_entities(self, entity_type: str | None = None, n: int = 5) -> list[Entity]:
        """获取最近的实体"""
        if entity_type:
    ...
    def get_last_entity(self, entity_type: str) -> Entity | None:
        """获取最后一个特定类型的实体"""
    ...
    def set_slot(self, name: str, value: Any, required: bool = True, prompt: str = "", validation_regex: str | None = None):
        """设置槽位"""
...
class ContextAwareDetector:
    ...
    def get_missing_slot_prompt(self, session_id: str) -> str | None:
        """获取缺失槽位的提示"""
        memory = self.get_memory(session_id)
...
class MultiTurnIntentProcessor:
    ...
    def __init__(self, detector, context_detector: ContextAwareDetector | None = None):
