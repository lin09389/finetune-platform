...
class IntentMetrics:
    ...
    def get_precision(self, intent: Optional[str] = None) -> float:
        """获取精确率"""
        if intent:
    ...
    def get_recall(self, intent: Optional[str] = None) -> float:
        """获取召回率"""
        if intent:
            tp = self.true_positives[intent]
    ...
    def get_f1_score(self, intent: Optional[str] = None) -> float:
        """获取F1分数"""
