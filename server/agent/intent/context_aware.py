...
class ContextManager:
    ...
    def resolve_reference(self, session_id: str, reference: str) -> str | None:
        """解析引用"""
        ctx = self.get_or_create_session(session_id)
...
class SessionStateTracker:
    ...
    def clear_state(self, session_id: str, key: str | None = None):
        """清除状态"""
        if key:
...
class ContextAwareDetector:
    ...
    def _predict_next_intent(self, ctx: ConversationContext) -> str | None:
        """预测下一个意图"""
...
def create_context_aware_detector(context_manager: ContextManager | None = None) -> ContextAwareDetector:
    """创建上下文感知检测器"""
