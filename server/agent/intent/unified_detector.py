...
class ConversationContext:
    ...
    def __init__(self, session_id: str):
        ...
        self.expecting_action: Optional[str] = None
        self.last_updated = datetime.now()
    
    def add_message(self, role: str, content: str, intent: Optional[str] = None, entities: Optional[Dict[str, Any]] = None):
        """添加消息"""
...
class UnifiedIntentDetector:
    ...
    def _infer_from_context(self, message: str, context: ConversationContext) -> Optional[IntentResult]:
        """从上下文推断意图"""
        action_keywords = {
            "读取": "file_read",
            "查看": "file_read",
    ...
    def _create_clarification(self, results: List[IntentResult], message: str) -> Optional[Dict[str, Any]]:
        """创建澄清对话"""
        if len(results) < 2:
            return None
    ...
    def detect(
        ...
        if self.bert_classifier and self.bert_classifier.is_loaded():
            try:
                ...
                if bert_result.intent != "unknown" and bert_result.confidence > 0.5:
