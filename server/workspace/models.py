...
class ProjectCreate(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(default_factory=dict, description="项目元数�?)
...
class ProjectUpdate(BaseModel):
    ...
    description: Optional[str] = Field(default=None, max_length=500, description="项目描述")
    tags: Optional[List[str]] = Field(default=None, description="项目标签")
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="项目元数�?)
    status: Optional[ProjectStatus] = Field(default=None, description="项目状�?)
...
class Project(BaseModel):
    ...
    tags: List[str] = Field(default_factory=list, description="项目标签")
    status: ProjectStatus = Field(default=ProjectStatus.ACTIVE, description="项目状�?)
    metadata: Dict[str, Any] = Field(default_factory=dict, description="项目元数�?)
...
class FileVersion(BaseModel):
    ...
    version_number: int = Field(..., ge=1, description="版本�?)
    ...
    created_at: str = Field(default_factory=lambda: datetime.now().isoformat(), description="创建时间")
    metadata: Dict[str, Any] = Field(default_factory=dict, description="版本元数�?)
...
class FileMetadata(BaseModel):
    ...
    lines: Optional[int] = Field(default=None, description="行数")
    checksum: Optional[str] = Field(default=None, description="校验�?)
...
class FileInfo(BaseModel):
    ...
    current_version: int = Field(default=1, description="当前版本�?)
    version_count: int = Field(default=1, description="版本总数")
    metadata: FileMetadata = Field(default_factory=FileMetadata, description="文件元数�?)
    created_at: str = Field(default_factory=lambda: datetime.now().isoformat(), description="创建时间")
    updated_at: str = Field(default_factory=lambda: datetime.now().isoformat(), description="更新时间")
...
class ProjectStatistics(BaseModel):
    ...
    latest_activity: Optional[str] = Field(default=None, description="最近活动时�?)
...
class FileUploadResult(BaseModel):
    ...
    is_new: bool = Field(default=True, description="是否新文�?)
...
class SubTask(BaseModel):
    ...
    title: str = Field(..., description="子任务标�?)
...
class TaskCreate(BaseModel):
    ...
    priority: TaskPriority = Field(default=TaskPriority.NORMAL, description="任务优先�?)
    due_date: Optional[str] = Field(default=None, description="截止日期 (ISO格式)")
    assignee: Optional[str] = Field(default=None, description="负责�?)
    tags: List[str] = Field(default_factory=list, description="任务标签")
    subtasks: List[SubTask] = Field(default_factory=list, description="子任务列�?)
    metadata: Dict[str, Any] = Field(default_factory=dict, description="任务元数�?)
...
class TaskUpdate(BaseModel):
    ...
    description: Optional[str] = Field(default=None, max_length=2000, description="任务描述")
    status: Optional[TaskStatus] = Field(default=None, description="任务状�?)
    priority: Optional[TaskPriority] = Field(default=None, description="任务优先�?)
    due_date: Optional[str] = Field(default=None, description="截止日期 (ISO格式)")
    assignee: Optional[str] = Field(default=None, description="负责�?)
    tags: Optional[List[str]] = Field(default=None, description="任务标签")
    subtasks: Optional[List[SubTask]] = Field(default=None, description="子任务列�?)
    progress: Optional[int] = Field(default=None, ge=0, le=100, description="进度百分�?)
    metadata: Optional[Dict[str, Any]] = Field(default=None, description="任务元数�?)
...
class Task(BaseModel):
    ...
    project_id: Optional[str] = Field(default=None, description="所属项目ID")
    status: TaskStatus = Field(default=TaskStatus.PENDING, description="任务状�?)
    priority: TaskPriority = Field(default=TaskPriority.NORMAL, description="任务优先�?)
    due_date: Optional[str] = Field(default=None, description="截止日期")
    assignee: Optional[str] = Field(default=None, description="负责�?)
    tags: List[str] = Field(default_factory=list, description="任务标签")
    subtasks: List[SubTask] = Field(default_factory=list, description="子任务列�?)
    progress: int = Field(default=0, ge=0, le=100, description="进度百分�?)
    metadata: Dict[str, Any] = Field(default_factory=dict, description="任务元数�?)
    created_at: str = Field(default_factory=lambda: datetime.now().isoformat(), description="创建时间")
    updated_at: str = Field(default_factory=lambda: datetime.now().isoformat(), description="更新时间")
    started_at: Optional[str] = Field(default=None, description="开始时�?)
    completed_at: Optional[str] = Field(default=None, description="完成时间")
    created_by: Optional[str] = Field(default=None, description="创建�?)
...
class TaskNotification(BaseModel):
    ...
    created_at: str = Field(default_factory=lambda: datetime.now().isoformat(), description="创建时间")
    recipient: Optional[str] = Field(default=None, description="接收�?)


class TaskProgress(BaseModel):
    """任务进度更新"""
    task_id: str = Field(..., description="任务ID")
    progress: int = Field(..., ge=0, le=100, description="进度百分�?)
...
class TaskStatistics(BaseModel):
    ...
    overdue_tasks: int = Field(default=0, description="逾期任务�?)
    high_priority_tasks: int = Field(default=0, description="高优先级任务�?)
    completion_rate: float = Field(default=0.0, description="完成�?)
