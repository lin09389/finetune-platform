"""
GitHub 项目分析技�?分析 GitHub 仓库，学习代码模式，提供改进建议
"""
import asyncio
import json
import re
    ...
    def _generate_recommendations(self, repo_info: Dict, structure: List[Dict], focus_area: str) -> List[Dict]:
        ...
        if language == "Python":
            recommendations.append({
                ...
                "suggestion": "建议使用 pyproject.toml 管理依赖，采�?src/ 目录结构",
                "priority": "medium",
            })
            recommendations.append({
                "category": "quality",
                "suggestion": "添加类型注解，使�?mypy 进行静态类型检�?,
                "priority": "high",
            })
            ...
            recommendations.append({
                "category": "architecture",
                "suggestion": "建议使用 monorepo 结构，配�?ESLint �?Prettier",
                "priority": "high",
            })
        ...
        if focus_area == "security":
            recommendations.extend([
                ...
                {"category": "security", "suggestion": "添加安全扫描工具 (�?Snyk, Safety)", "priority": "medium"},
            ])
...
class CodePatternSkill(SkillBase):
    ...
    def _detect_patterns(self, code: str, language: str) -> List[Dict]:
        ...
        if "TODO" in code or "FIXME" in code:
            patterns.append({"name": "todo_comments", "description": "代码中包含待办事项注�?})
        
        if "print(" in code or "console.log" in code:
            patterns.append({"name": "debug_print", "description": "代码中包含调试打印语�?})
        
        if re.search(r"password\s*=\s*['\"]", code, re.IGNORECASE):
            patterns.append({"name": "hardcoded_secret", "description": "代码中可能包含硬编码的密�?, "severity": "high"})
    ...
    def _generate_suggestions(self, patterns: List[Dict], language: str) -> List[str]:
        ...
        for pattern in patterns:
            if pattern["name"] == "debug_print":
                suggestions.append("建议移除调试打印语句，使用日志框架代�?)
            elif pattern["name"] == "hardcoded_secret":
                suggestions.append("警告：请使用环境变量或配置文件管理敏感信�?)
