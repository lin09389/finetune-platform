...
class SkillParameter(BaseModel):
    ...
    default: Optional[Any] = Field(default=None, description="默认�?)
    enum: Optional[List[Any]] = Field(default=None, description="枚举值列�?)
...
class SkillMetadata(BaseModel):
    ...
    display_name: str = Field(..., description="技能显示名�?)
    description: str = Field(..., description="技能详细描�?)
    version: str = Field(default="1.0.0", description="技能版�?)
    category: SkillCategory = Field(default=SkillCategory.CUSTOM, description="技能类�?)
    tags: List[str] = Field(default_factory=list, description="技能标�?)
    ...
    dependencies: List[str] = Field(default_factory=list, description="依赖的其他技�?)
    ...
    requires_auth: bool = Field(default=False, description="是否需要认�?)
    requires_confirmation: bool = Field(default=False, description="是否需要用户确�?)
    dangerous: bool = Field(default=False, description="是否为危险操�?)
...
class SkillResult(BaseModel):
    ...
    metadata: Dict[str, Any] = Field(default_factory=dict, description="额外元数�?)
    ...
    tokens_used: Optional[int] = Field(default=None, description="使用�?token 数量")
    created_at: datetime = Field(default_factory=datetime.now, description="创建时间")
...
class SkillExecution(BaseModel):
    ...
    status: SkillStatus = Field(default=SkillStatus.PENDING, description="执行状�?)
    priority: SkillPriority = Field(default=SkillPriority.NORMAL, description="执行优先�?)
    result: Optional[SkillResult] = Field(default=None, description="执行结果")
    started_at: Optional[datetime] = Field(default=None, description="开始时�?)
    completed_at: Optional[datetime] = Field(default=None, description="完成时间")
    created_at: datetime = Field(default_factory=datetime.now, description="创建时间")
    user_id: Optional[str] = Field(default=None, description="用户ID")
    session_id: Optional[str] = Field(default=None, description="会话ID")
    parent_execution_id: Optional[str] = Field(default=None, description="父执行ID（用于链式调用）")
    retry_count: int = Field(default=0, description="已重试次�?)
...
class SkillChain(BaseModel):
    ...
    name: str = Field(..., description="链名�?)
    description: str = Field(default="", description="链描�?)
...
class SkillValidationResult(BaseModel):
    ...
    normalized_params: Optional[Dict[str, Any]] = Field(default=None, description="规范化后的参�?)
